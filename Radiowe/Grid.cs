...
namespace Radiowe
{
    public class Grid : IPrint
    {
        ...
        public Cell[,] AddFirstBaseStation(BaseStation station) 
        {
            ...
            // DODAWANIE PIERWSZEJ STACJI DO SYSTEMU 
            if (first_)
            {
                ...
                for (int i = 0; i < kSize; i++)
                {
                    for (int j = 0; j < kSize; j++)
                    {
                        ...
                        double temp_3w = calculations_.CalculateReceiverPower(station.GetPower(), station.GetGain(), station.antenna_gain_receiver_); 
                        ...
                        if (i == station.GetLocationX() && j == station.GetLocationY())
                        {
                            ...
                            grid_[i, j].AddToList2(station.name_, station.GetPower() + station.GetGain(), station.GetPower() + station.GetGain(), station.channel_); 
                        }
                        else if (SNRw >= 6)
                        {
            ...
            for (int i = 0; i < kSize; i++)
            {
                for (int j = 0; j < kSize; j++)
                {
                    if (grid_[i, j].GetSINR(station_in_sys.channel_) > 6 && grid_[i, j].GetName(station_in_sys.channel_) == station_in_sys.name_)
                    {
                        ...
                        double temp6w = calculations_.CalculateSINR(channel_diff); 
        ...
        public bool CheckUserParameter(BaseStation station)
        {
            ...
            double temp_3 = calculations_.CalculateReceiverPower(station.GetPower(), station.GetGain(), station.antenna_gain_receiver_); 
